export default {
    port: 1337,
    host: '0.0.0.0',
    dbUri: "mongodb://database:27017/?readPreference=primary&appname=MongoDB%20Compass&directConnection=true&ssl=false", //Connection string to database. 
    appUri: "http://localhost", //Url used for verification emails.
    saltWorkFactor: 10, // Salt used for cypher the passwords
    accessTokenTtl: "15m", //Token to login 
    refreshTokenTtl: "1y", //Token to refresh login
    privateKey: `-----BEGIN RSA PRIVATE KEY-----
    
    -----END CERTIFICATE-----`,
    email: '', //Email on gmail for notifications
    emailpass: '', //Password for email
};
